...
public class TestConfigCenterClient {
  ...
  public void testConnectServer(@Mocked ClientPoolManager<HttpClientWithContext> clientMgr) {
    ...
    Mockito.when(event.statusCode()).thenReturn(200);
    Mockito.when(event.body()).thenReturn(Future.succeededFuture(rsp));
  ...
  // TODO: config center code should be unified to clients next release, so do not recover test codes for this
//  @Test
...
//  }
  ...
  private String refreshAndGetCurrentRevision(ClientPoolManager clientMgr, int statusCode, String version) {
    ...
    Mockito.when(
        httpClient.request(Mockito.any(HttpMethod.class), Mockito.anyInt(), Mockito.anyString(), Mockito.anyString()))
        .thenReturn(Future.succeededFuture(request));
    Mockito.when(request.send()).thenReturn(Future.succeededFuture(httpClientResponse));
    Mockito.when(httpClientResponse.statusCode()).thenReturn(200);
    Mockito.when(httpClientResponse.body()).thenReturn(Future.succeededFuture(rsp));
...
//            , Mockito.anyInt(), Mockito.anyString(), Mockito.anyString())).thenReturn(Future.succeededFuture(request));
//
//    Mockito.<Void>when(httpClient.webSocket(Mockito.any(WebSocketConnectOptions.class),
